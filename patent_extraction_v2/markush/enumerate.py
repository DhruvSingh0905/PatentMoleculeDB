...
from ..core.models import MarkushFormula, Compound, CompoundSource, IupacSource
from .mapper import lookup_substituent, SUBSTITUENT_LIBRARY
...
def enumerate_markush(
    ...
    - "decomposition": reconstruct from RGroupDecompose results
    - "scored": novel combos ranked by frequency + co-occurrence

    Args:
        scaffold_index: Optional caller-assigned index for the scaffold being
            enumerated; carried into Compound.provenance for traceability.
    ...
    if mode == "specified" and r_value_table:
        ...
        for rank, row in enumerate(r_value_table[:cap], start=1):
            smiles = _instantiate_core(markush.core_smiles, row)
            if smiles:
                compound = _validate_and_build(
                    markush.patent_id, smiles, row,
                    scaffold_smiles=markush.core_smiles,
                    scaffold_index=scaffold_index,
                    enumeration_mode="specified",
                    scoring_rank=rank,
                )
                if compound:
        ...
        seen_smiles = set()
        for rank, result in enumerate(decomposition_results[:cap], start=1):
            ...
            if reconstructed and reconstructed not in seen_smiles:
                ...
                compound = _validate_and_build(
                    markush.patent_id, reconstructed,
                    {k: v[:30] for k, v in assignments.items()},
                    scaffold_smiles=markush.core_smiles,
                    scaffold_index=scaffold_index,
                    enumeration_mode="decomposition",
                    scoring_rank=rank,
                )
                if compound:
                    compounds.append(compound)
        ...
        for neg_score, _, smi_assignments in scored_combos:
            ...
            if smiles and smiles not in seen_smiles:
                ...
                compound = _validate_and_build(
                    markush.patent_id, smiles, smi_assignments,
                    scaffold_smiles=markush.core_smiles,
                    scaffold_index=scaffold_index,
                    enumeration_mode="scored",
                    scoring_rank=rank_iter,
                )
                if compound:
                    compounds.append(compound)
        ...
        for combo in product(*r_options):
            ...
            if smiles:
                compound = _validate_and_build(
                    markush.patent_id, smiles, assignments,
                    scaffold_smiles=markush.core_smiles,
                    scaffold_index=scaffold_index,
                    enumeration_mode=mode,    # "sample" or "exhaustive"
                    scoring_rank=count + 1,
                )
                if compound:
                    compounds.append(compound)
...
def _validate_and_build(
    ...
    scoring_rank: int | None = None,
) -> Compound | None:
    """Validate SMILES and build Compound object with structured Markush provenance."""
    ...
    # Structured provenance — captures scaffold + R-assignments + scoring rank
    # so downstream can answer "which scaffold filled in this Example?"
    from ..core.models import CompoundProvenance
    scaffold_inchikey = None
    if scaffold_smiles:
        sk = get_inchikey(canonicalize_smiles(scaffold_smiles) or scaffold_smiles)
        if sk:
            scaffold_inchikey = sk[:14]   # connectivity-key prefix
    prov = CompoundProvenance(
        route="markush_enumeration",
        stage=enumeration_mode or "markush_enumeration",
        scaffold_inchikey=scaffold_inchikey,
        scaffold_index=scaffold_index,
        r_assignments=r_assignments,
        enumeration_mode=enumeration_mode,
        scoring_rank=scoring_rank,
        chain=["markush_enumeration"],
    )
