...
JSON, but whether the rule is *correct* is unknowable from the response — and
unknowable before it runs, which is why nothing here tries. `loop.py` applies
the proposal to the patent it was bought for and `outcome.measure` reports what
came out; a rule that fails is handed its own measurement and asked again, up
to `MAX_ATTEMPTS`.

That retry is why this file takes `attempts` and `digest`. A second call with
an identical prompt at temperature 0 is the same call, so a retry is only worth
paying for if the question has changed — and the thing that changed is that we
now know what the last rule DID.
"""
...
# Defined in `core/config` with the rest of the cost knobs; re-exported here
# because `loop` imports it from this module. See config.SYNTH_MODEL for why
# Haiku.
...
that matches "some number somewhere in the row" will pull values out of the \
molecular-weight and m/z columns, which is measured and fails.
...
— return `value_pattern` with a regex for ONE CELL. It is only ever applied to \
cells our parser already failed on, so it cannot change a value we read \
correctly; write it to match the cells you were shown as unreadable.
...
YOUR JOB IS TO DESCRIBE, NOT TO DECIDE. Every rule you return is RUN on this \
patent before anything is kept, and what it produces is measured: it must \
produce at least one record, it must not contradict a value we already read \
correctly, and it must not take a value from a column that reads as a molecular \
weight, an m/z, an NMR shift or a retention time. A rule that fails is not \
adopted, and you are told exactly what it did and asked again. Nothing you \
propose can reach the database without passing that.

A missing unit does NOT fail a rule — omit `unit` when the table does not state \
one rather than supplying the unit this assay usually carries. We check every \
unit you give against this document's own text, so a unit that is conventional \
but unstated is dropped, while one written anywhere in the header, caption, \
legend or cells is kept whatever its form — `min`, `µL/min/mg protein` and \
`min·ng/mL` are all fine.

So there is no option here for "this is awkward" or "I am unsure", and no \
penalty for being wrong — a wrong rule costs one more attempt, and a missing \
rule costs the data. Report what the fragment shows. If two readings are \
possible, return the more likely one. If part of the layout defeats you, \
describe the part you DO understand — a rule covering three of five columns is \
worth more than nothing, and the columns that hold up are kept."""
...
def _feedback(attempts: list[dict]) -> str:
    """What earlier attempts at THIS layout did, so the model can correct.

    The single change that makes a retry worth paying for. Without it, attempt
    two is attempt one with a different random seed — the same prompt, the same
    temperature 0, the same answer. With it the model is answering a different
    question: not "how would you read this table" but "your rule produced no
    records / took a value from the molecular-weight column; here it is again".

    Deliberately reports the OUTCOME and never suggests a fix. The measurement
    is ours and is not in doubt; what to do about it is the model's job, and a
    hint from us is a hypothesis dressed as evidence — it would be believed
    because of where it came from rather than because it is true.
    """
    if not attempts:
        return ""
    lines = [
        f"\nYOU HAVE ALREADY TRIED {len(attempts)} RULE(S) ON THIS EXACT TABLE. "
        f"Each was run on the patent and measured. Here is what happened — do "
        f"not repeat a rule that has already failed:"]
    for i, a in enumerate(attempts, 1):
        lines.append(f"\n  ATTEMPT {i} — {a.get('kind')}")
        lines.append(f"    you proposed: {json.dumps(a.get('payload'), default=str)[:600]}")
        lines.append(f"    what it did:  {a.get('why')}")
    lines.append(
        "\nThat is a measurement of your rule against this patent, not an "
        "opinion about it. Diagnose it yourself and answer again.\n")
    return "\n".join(lines)
...
def propose(gap: Gap, *, model: str = SYNTH_MODEL, patent_id: str = "",
    ...
    source_block = (
        f"THE PATENT'S OWN XML FOR THIS TABLE — the source of truth. Character "
        f"entities have been resolved, so units read as themselves (μM, °C, ±). "
        f"`&lt;` and `&gt;` are left as-is because they are markup elsewhere; "
        f"inside a cell they are literal < and >, the qualifiers on a value.\n"
        f"{raw}\n\n"
    ...
    # The attempt history is part of the QUESTION, so it has to be part of the
    # key. Without it every retry reads attempt one's cached answer back and the
    # loop spins three times over the same rule at $0 — which looks exactly like
    # a model that will not change its mind.
    attempt_key = ("::a" + hashlib.sha256(
        json.dumps([a.get("why") for a in attempts], default=str).encode()
    ).hexdigest()[:8]) if attempts else ""
    cache_key = f"synth::{PROMPT_VERSION}::{gap.fingerprint}::{model}{attempt_key}"
    ...
    for turn in range(MAX_TURNS):
        ...
        usage = resp.usage
        # The cache buckets are SEPARATE from `input_tokens` and are billed —
        # reads at 0.1x, writes at 1.25x. This call site already read
        # `cache_read_input_tokens` for the log line below and passed only
        # `input_tokens` to the tracker, so every cached token cost $0 in our
        # accounting while costing real money. Under-reporting is the worse
        # direction: a spend cap cannot trip on tokens it never sees.
        cost_tracker.record(
