...
Why this exists
---------------
Every other source considered for this pipeline reconstructs structure that
USPTO already published. Google Patents renders clean text but drops `<table>`
structure; MinerU recovered table structure by OCR-ing a PDF, paying for it with
`<|ref|>` tags, `[[bbox]]` pollution, merged rows and character errors (`2-y1}`
for `2-yl}`). USPTO's own grant XML carries the tables as OASIS/CALS markup —
exact cells, no OCR, qualifiers intact. Neither alternative is in v3; this is
the only source. See `sources/__init__` for what that costs.
...
Coverage
--------
`PTGRXML` begins in **2002** — there is no grant full-text XML before that. In
v2 older grants fell through to the Google Patents tier. THERE IS NO SUCH TIER
IN v3: `fetch_grant_xml` raises `UsptoUnavailable` and the patent is skipped
outright. The skip is always printed, never silent.
...
# ── fetch ─────────────────────────────────────────────────────────
#
# THIS DID NOT COME ACROSS IN THE v3 MOVE and the omission was invisible for as
# long as every patent we touched was already cached. v3 could parse a grant XML
# and could not obtain one, so the corpus was frozen at whatever 137 documents
# happened to be on disk — a package that can only read what someone else
# fetched is not a pipeline.
#
# One search call to locate the document, then one ~600 KB GET for its own XML.
# The search response carries a direct per-document file URI, so there is no
# bulk download anywhere in this.
_SEARCH_URL = "https://api.uspto.gov/api/v1/patent/applications/search"
_CACHE_DIR = config.XML_INPUT_DIR
_TIMEOUT_S = 90
_MAX_RETRIES = config.MAX_RETRIES


class UsptoUnavailable(Exception):
    """No grant XML exists for this patent, or the API would not serve it."""
...
# data set at all); `_is_namelike` uses the NARROW list already in place here
# (rejecting a name-as-id row from being read as a header). Kept as two named
# constants rather than merged into one — merging would change which rows
# each caller accepts.
_CHEM_SUBSTR_WIDE = re.compile(
    r'(?:yl|phenyl|methyl|imidazol|pyrimidin|morpholin|triazol|'
    r'chloro|bromo|fluoro|ethyl|propyl|cyclo|oxo|amino|nitro|'
    r'benz|piperid|piperaz|pyridine|pyrazol|oxazol|thiazol)', re.IGNORECASE)
_CHEM_SUBSTR_NARROW = re.compile(
    r'(?:yl|phenyl|methyl|imidazol|pyrimidin|morpholin|triazol|'
    r'chloro|bromo|fluoro|ethyl|propyl|cyclo)', re.IGNORECASE)


def _looks_like_chem_name(name: str, pattern: re.Pattern) -> bool:
    """A long string with chemical-name substrings or heavy punctuation.

    The one boolean shape both `_opens_with_id` and `_is_namelike` computed
    inline, each against its own substring list (see `_CHEM_SUBSTR_WIDE` /
    `_CHEM_SUBSTR_NARROW` above): the pattern search, OR 3-or-more of the
    punctuation marks `-`/`[`/`(` a systematic name tends to carry. Which
    pattern applies is the caller's choice — this only removes the repeated
...
def _row_filled(row: list["Cell"]) -> bool:
    """Does this row carry any non-blank cell text?

    `any(c.text.strip() for c in r)` appears four times in `assemble_block`/
    `assembly_fidelity` — the same predicate asked of the same shape of row —
    factored here rather than repeated.
    """
    ...
    if not declared and len(texts) == 2:
        ...
        for idx in (0, 1):
            ...
            if re.fullmatch(r'[<>=~≤≥]*\s*\d+\.?\d*', val) and len(name) > 15:
                ...
                if _looks_like_chem_name(name, _CHEM_SUBSTR_NARROW):
                    return False
...
def assemble_block(tables: list["Table"], table_id: str) -> "Table | None":
    ...
    kin, filled = [], 0
    for t in same:
        n = sum(1 for r in t.body_rows if _row_filled(r))
        if not n:
            continue
        filled += n
    ...
    if kin:
        ...
        for t in same:
            rows_by_width[t.n_cols] = rows_by_width.get(t.n_cols, 0) + sum(
                1 for r in t.body_rows if _row_filled(r))
...
def assembly_fidelity(xml: str) -> list[dict]:
    ...
    for tid in seen:
        ...
        n_body = sum(1 for r in asm.body_rows if _row_filled(r))
