...
def _hash_key(model: str, prompt: str, image_data: str | None = None) -> str:
    ...
    The image is hashed in FULL. An earlier version hashed only
    `image_data[:1000]` "for speed", which silently collided: two
    different crops sharing a base64 prefix mapped to one key, so the
    second one was served the first one's SMILES. Measured on the 412
    crops in `output_v2/images`, one such collision was already present.
    Hashing 1 KB instead of ~50 KB saves microseconds and costs
    correctness — never reintroduce the truncation.
    """
    h = hashlib.sha256()
    h.update(model.encode())
    h.update(prompt.encode())
    if image_data:
        h.update(image_data.encode())
