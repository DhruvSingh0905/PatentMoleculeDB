...
def _run_mineru(pdf_path: Path, out_dir: Path) -> Path:
    ...
    # MinerU is slow (5-10 min per patent); stream output for diagnostics.
    result = subprocess.run(
        cmd, capture_output=True, text=True, timeout=1200,  # 20 min
    )
...
def run_mineru_for_patent(
    ...
    *,
    ...
    Idempotent: skips MinerU if `all_pages/` is already populated.
    Pass `force=True` to override and re-OCR.
    """
