...
from ..core.models import MarkushContext
...
def get_context_for_prompt(context: MarkushContext) -> str:
    ...
    Migrated from the deleted extract_markush_context module. Used by
    iupac_to_smiles to inject scaffold/R-group context into name-resolution
    prompts so the LM gets the patent-specific molecular family hint.

    Returns the prompt fragment, or "" if quality is "failed".
    """
    ...
    return ("This compound belongs to a patent with the following molecular "
            "scaffold:\n" + "\n".join(parts))
...
def _symbolic_r_group_parse(patent_id: str, data_dir: Path) -> dict[str, list[str]]:
    ...
    for pf in sorted(pages_dir.glob('page_*.md')):
        text = pf.read_text()
        text = re.sub(r'<[^>]+>', ' ', text)
        # Also strip PaddleX layout-marker residues that the HTML
        # strip above leaves behind: "text [[x1,y1,x2,y2]]" sequences
        # appear between R-label definitions in claim text and break
        # the regex's semicolon/comma boundary detection. See
        # routes/text_markush.py:_strip_paddlex_layout_artifacts for
        # the same fix with full commentary.
        text = re.sub(
        ...
        # Terminator uses lookahead so the next R-label boundary isn't
        # consumed (next finditer iteration matches it). Includes the
        # COMMA+R-label pattern, which is the dominant separator in
        # claim text (verified across 3 image-heavy patents in the
        # text_markush survey: 35 commas vs 16 semicolons vs 1 period).
        # See routes/text_markush.py:_RGROUP_DEF_RE for the same pattern
        ...
        for m in re.finditer(
            ...
            r'(?='
                ...
                r'|\s*[,;]\s+(?:and\s+)?(?:R\d+[a-z]?|[GXY])\s+is\b'         # next R-label (with optional "and" connector)
                r'|\s*;\s+[A-Z]'                                            # semicolon clause boundary (general)
                r'|\s*\.\s*$'                                               # paragraph end
            r')',
            text, re.DOTALL
        ):
...
def extract_markush_multiagent(
    ...
    from .mapper import derive_scaffold_from_examples, score_scaffold
    ...
    if example_smiles and len(example_smiles) >= 5:
        logger.info(f"  Layer 1: Deriving scaffolds from {len(example_smiles)} examples...")
        from .mapper import derive_all_scaffolds_from_examples
        all_scaffolds = derive_all_scaffolds_from_examples(example_smiles, min_match=5, max_scaffolds=10)

        if all_scaffolds:
            best = all_scaffolds[0]
            scaffold_smiles = best['scaffold_smiles']
            data_rgroup_libs = best['rgroup_libraries']
            if best['decomposition_results']:
                for i, key in enumerate(sorted(k for k in best['decomposition_results'][0].keys() if k != 'Core')):
                    lm_position_mapping[key] = str(i + 1)

            # Cache ALL scaffolds for multi-series enumeration
            cache_data = {
                "scaffold_smiles": scaffold_smiles,
                "score": best['match_count'],
                "position_mapping": lm_position_mapping,
                "rgroup_libraries": best['rgroup_libraries'],
                "all_scaffolds": [
                    {
                        "scaffold_smiles": s['scaffold_smiles'],
                        "match_count": s['match_count'],
                        "murcko_count": s['murcko_count'],
                        "rgroup_libraries": s['rgroup_libraries'],
                        "total_rgroup_options": s['total_rgroup_options'],
                    }
                    for s in all_scaffolds
                ],
                "source": "example_derived",
                "description": f"Derived {len(all_scaffolds)} scaffolds from {len(example_smiles)} examples",
            }
            cache_path.write_text(json.dumps(cache_data, indent=2, default=list))
            total_covered = sum(s['murcko_count'] for s in all_scaffolds)
            logger.info(
                f"  Layer 1: SUCCESS — {len(all_scaffolds)} scaffolds covering "
                f"{total_covered}/{len(example_smiles)} examples"
            )
    ...
    from .mapper import resolve_position_mapping
    from ..core.models import MarkushFormula, RGroupDef
